...
$stepList = [
sprintf(__('Connect %s', 'trustindex-plugin'), 'Yelp'),
...
<h1 class="ti-header-title"><?php echo sprintf(__('Connect %s', 'trustindex-plugin'), 'Yelp'); ?></h1>
...
<?php else: ?>
<div class="ti-box">
<form method="post" action="" data-platform="yelp" id="ti-connect-platform-form">
...
$labelText = sprintf(__('%s Business URL', 'trustindex-plugin'), 'Yelp');
$infoText = __("Type your business/company's URL and select from the list", 'trustindex-plugin');
$errorText = sprintf(__('Please add your URL again: this is not a valid %s page.', 'trustindex-plugin'), "Yelp");
...
<?php endif; ?>
<h1 class="ti-header-title ti-mt-2"><?php echo sprintf(__('Check some %s widget layouts and styles', 'trustindex-plugin'), 'Yelp Business reviews'); ?></h1>
...
<p>
<?php echo sprintf(__('There are no reviews on your %s platform.', 'trustindex-plugin'), 'Yelp'); ?>
</p>
</div>
<?php endif; ?>
